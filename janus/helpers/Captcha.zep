...
class Captcha
{
    public function draw()
    {
        ...
        let encrypted = openssl_encrypt(
            "_JANUSCAPTCHA=" . captcha . "=" . (time() + 1 * 60),
            "aes128",
            captcha,
			0,
			iv
		...
		return "<div class=\"janus-captcha-img\"><img src=\"data:image/png;base64," . base64_encode(image_data) . "\" alt=\"captcha\"/></div>" .
			"<input name=\"janus_captcha\" class=\"janus-captcha-input\" required/>" .
			"<input name=\"_JANUSCAPTCHA\" type=\"hidden\" value=\"" . encrypted . "\"/>";
	}

    public function validate()
	{
		if (!isset(_REQUEST["janus_captcha"]) || !isset(_REQUEST["_JANUSCAPTCHA"])) {            
			return false;
		}

		var splits, iv, encrypted, token;
		let splits = explode("=", _REQUEST["_JANUSCAPTCHA"]);
        ...
        if (splits[0] != "_JANUSCAPTCHA") {
            return false;
        }

        if (splits[1] != _REQUEST["janus_captcha"]) {
            return false;
        }
